...
import { add_remove } from "../../reducers/favourite";
...
export default function HomePage() {
  ...
  const listFavourite = useSelector((state) => state.favourite);
      ...
      <Container className={search ? "mt-0" : "mt-[50px]"}>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 xl:grid-cols-6 gap-10 my-10">
            {list.map((item, index) => {
              const listIdFavourite = listFavourite.map(
                (itemFav) => itemFav.id
              );
              const isFavourite = listIdFavourite.includes(item.id);
                  ...
                  {user && !loadingUser && (
                    <button
                      className={cn(
                        ...
                        isFavourite ? "text-red-500 border-red-400" : "text-white"
                      )}
                      onClick={() =>
                        dispatch(add_remove({ data: item, type: "home" }))
                      }
                    >
